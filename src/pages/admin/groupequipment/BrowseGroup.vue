<template>
  <div class="mx-auto flex flex-wrap p-5 flex-col md:flex-row">
    <div class="md:col-span-2 xl:col-span-3">
      <h3 class="text-lg font-semibold">Group Equipment</h3>
    </div>
    <div class="w-full flex flex-col mt-12">
            ...
            <table class="min-w-full te xt-left text-sm font-light" v-if="mode == 'browse'">
              <thead class="border-b font-medium dark:border-neutral-500">
                <tr>
                  ...
                  <th scope="col" class="px-6 py-4">Nama grup</th>
              ...
              <tbody>
                <tr v-for="(a, index) in dataGroup" :key="a.id" class="border-b dark:border-neutral-500">
      ...
      <div>
        <!--- Tempat Form Input -->
        <form-group ref="formGroup" @finish="onFormClose" />
      </div>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  methods: {
    ...
    onSelect(data) {
      this.mode = "form";
      this.$refs["formGroup"].selectData(data);
    },
    onSearching(event) {
      this.loadData();
    },
    ...
    loadData() {
      api.get("groupequipment", this.queryData).then((respon) => {
        this.dataGroup = respon.data.records;
