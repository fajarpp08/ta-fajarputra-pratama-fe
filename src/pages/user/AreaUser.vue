<template>
  <div class="bg-white py-0">
    ...
    <div class="w-full inline-flex py-4 px-6">
      <input v-model="queryData.name" @keyup="onSearching()"  placeholder="Masukan nama area"
    ...
    <table class="min-w-full text-left text-sm font-light">
      ...
      <tbody>
        <tr v-for="(a, index) in dataArea" :key="a.id" class="border-b dark:border-neutral-500">
...
export default defineComponent({
  ...
  methods: {
    ...
    loadData() {
      api.get("area/user", this.queryData).then((respon) => {
    ...
    onSelect(data) {
      this.$router.push("/groupequipmentuser");
    },
  },
});
</script>
