<template>
  <div class="bg-white py-0">
    ...
    <div class="w-full inline-flex py-4 px-6">
      <input v-model="queryData.name" @keyup="onSearching()" placeholder="Masukan nama tipe"
    ...
    <table class="min-w-full text-left text-sm font-light">
      ...
      <tbody>
        <tr v-for="(a, index) in dataType" :key="a.id" class="border-b dark:border-neutral-500">
          ...
          <td class="whitespace-nowrap px-6 py-4">{{ a.equipment.name }}</td>
          ...
          <td class="whitespace-nowrap px-6 py-4">{{ a.content }}</td>
          <td>
...
export default defineComponent({
  ...
  methods: {
    ...
    loadData() {
      api.get("type/user", this.queryData).then((respon) => {
